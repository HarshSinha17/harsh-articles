...
module.exports = {
    ...
    createOrganization: async (req, res) => {
        try {
            const { name } = req.body;
            const organization = await Organization.createOrganization({ name });
            const organizationsJson = await Organization.objectForResponse(organization);
            res.json(organizationsJson);
        } catch (e) {
            logger.error('Organization Admin: [createOrganization]', { message: e.message });
    ...
    deleteOrganization: async (req, res) => {
        try {
            ...
            if ((await Project.findNonDraftOrganizationProjectIds(organizationId)).length > 0) {
                throw new errorUtils.ResponseError('OrganizationHasNonDraftProjects');
            }
            ...
            logger.error('Organization Admin: [deleteOrganization]', { message: e.message });
            res.status(e.status || 500).json({ message: e.message });
        }
    }
};
