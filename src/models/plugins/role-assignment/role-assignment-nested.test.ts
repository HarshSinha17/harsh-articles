...
import { NestedRoleAssignmentPlugin } from './role-assignment-nested';
...
describe('Role assignment nested plugin', () => {
    ...
    test('user with role should pass isAuthorized for the assigned and default roles"', async () => {
        ...
        const roleAssignment = (await TestOrgModel.findOne({ _id: testOrgObject._id! }))!.tests![0]!.accessControlList![0]!;
        ...
        expect((await TestOrgModel.findOne({ _id: testOrgObject._id! }))!.tests![0]!.accessControlList).toHaveLength(0);
    });
