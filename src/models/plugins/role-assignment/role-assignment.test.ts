...
describe('Role assignment plugin', () => {
    ...
    let TestModel: mongooseType.Model<ITestDoc> & RestrictedModel<ITestDoc, TestRoles>;
    let TestSchema;
    ...
    test('user with role should pass isAuthorized for the assigned and default roles"', async () => {
        ...
        // Reflacted in DB
        const roleAssignment = (await TestModel.findOne({ _id: testObject._id! }))!.accessControlList![0]!;
        ...
        // Reflacted in DB
        expect((await TestModel.findOne({ _id: testObject._id! }))!.accessControlList).toHaveLength(0);
    });
