...
describe('Organization RegisteredThemes SubModel', () => {
    ...
    test('statics.createRegisteredTheme', async () => {
        ...
        expect(organization!.registeredThemes![0]!.accessControlList).toHaveLength(1);
        // admin role granted to creator
        expect(organization!.registeredThemes![0]!.accessControlList![0]!).toEqual(
            expect.objectContaining({
                _id: expect.any(mongoose.Types.ObjectId),
                userId: creatorUser._id,
                role: 'REGISTERED_THEME_ADMIN'
            })
        );
        ...
        expect(
            await getThrownError(() =>
                Organization.createRegisteredTheme(new mongoose.Types.ObjectId(), validRegisteredThemeMockData, creatorUser._id!)
            )
    ...
    test('static.updateRegisteredTheme', async () => {
        ...
        const organization = await Organization.createRegisteredTheme(org._id!, { ...validRegisteredThemeMockData }, creatorUser._id!);
