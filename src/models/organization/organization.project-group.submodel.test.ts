...
import type { default as UserType, IUser, IUserDoc } from '../user.model';
...
describe('Organization ProjectGroups SubModel', () => {
    ...
    beforeEach(async () => {
        ...
        creatorUser = await fetchUser(user._id!);
    });
    ...
    test('statics.createProjectGroup', async () => {
        ...
        const organization = await Organization.createProjectGroup(createdOrganization._id!, validProjectGroupMockData, creatorUser._id!);
        expect(organization!.projectGroups).toHaveLength(1);

        // admin role granted to creator
        expect(organization!.projectGroups![0]!.accessControlList![0]!).toEqual(
        ...
        expect(
            await getThrownError(() =>
                Organization.createProjectGroup(new mongoose.Types.ObjectId(), validProjectGroupMockData, creatorUser._id!)
            )
    ...
    test('static.updateProjectGroup', async () => {
        ...
        const organization = await Organization.createProjectGroup(org._id!, { ...validProjectGroupMockData }, creatorUser._id!);
    ...
    describe('Access Control List', () => {
        ...
        beforeEach(async () => {
            ...
            const organization = await Organization.createProjectGroup(org._id!, { name: 'test project group' }, creatorUser._id!);
            const createdProjectGroup = (await organization!.getProjectGroups())![0]!;
