...
class GitBasedCms {
    ...
    createPage(project, user, { modelName, schema, srcProjectId, srcEnvironment, fields, pageModel }) {
        ...
        if (!model || !pageModel.filePath) {
            throw new Error('Model should be defined, and pageModel should contains filePath attribute');
        }

        const slug = userSlug || generateNameId(modelName);
        const data = this.createObject(slug, otherFields, { model, pageModel, schema, cmsId });

        logger.debug('GitBased: create page', { srcProjectId: srcProjectId, projectId: project.id, userId: user.id, modelName, filePath: data.filePath, slug });
    ...
    async getStagedChanges(project, user, environmentName) {
        return containerService.getStagedChanges(project, user, environmentName);
    }
