...
module.exports = {
        ...
        if (project.wizard?.deployment?.settings?.sharedUser) {
            ...
            if (projectPartialUpdate.customDomain !== undefined) {
                ...
                if (projectPartialUpdate.customDomain === null) {
                    ...
                    updatedSite = await netlifyService.updateSite(siteId, { custom_domain: `${netlifySite.name}.${config.netlify.shared.domain}` }, netlifyAccessToken);
                } else {
                    updatedSite = await netlifyService.updateSite(siteId, { custom_domain: projectPartialUpdate.customDomain }, netlifyAccessToken);
                }
                project = await netlifyService.updateProjectFromNetlifySiteOrDeploy(updatedSite, project);
            }
    ...
    verifyCustomDomain: async function(project, user) {
        ...
        if (project.customDomain && project.defaultDomain) {
            const { siteId, netlifyAccessToken } = await getNetlifySiteIdAndToken(project, user);
            const result = await netlifyService.getSiteSslCert(siteId, netlifyAccessToken);
            if (result?.state === 'issued' && result?.domains?.length === 1 && result?.domains[0] === project.customDomain && !result?.renewal_error_message) {
                return { status: CUSTOM_DOMAIN_STATUSES.VERIFIED };
            }
            return { status: CUSTOM_DOMAIN_STATUSES.PENDING_VERIFICATION };
        }
        return { status: CUSTOM_DOMAIN_STATUSES.NOT_VERIFIED };
