...
function getTaskDefinitionForProject(project: IProjectDoc) {
    ...
    const allPrepackagedTaskDefinitions = config.container.shared.prepackagedTaskDefinitions as Record<string, string>; // TODO: move this cast to build-config in single-config v2
    let prepackagedTaskDefinition = ssg && allPrepackagedTaskDefinitions[ssg];
    if (ssg && project.wizard?.settings?.studioVersion === 2) {
        prepackagedTaskDefinition = allPrepackagedTaskDefinitions[ssg + '-v2'] || prepackagedTaskDefinition;
    }
