...
async function createStackbitWebhook(project, token, eventName =  'github', events = ['push', 'delete', 'pull_request', 'repository']) {
    ...
    const { ownerLogin, repoName, id } = repoDetails;
    let shouldGenerateCreateWebhook = true;
    let webhookId = uuid();

    if (project.wizard?.theme?.settings?.multiSite) {
        const projectsWithRepoId = (await Project.findProjectsByRepoId('github', id))?.filter(({ id }) => id !== project.id);
        const existingWebhookId = projectsWithRepoId?.[0]?.webhooks?.github?.id;
        if (existingWebhookId) {
            webhookId = existingWebhookId;
            shouldGenerateCreateWebhook = false;
        }
    }

    if (shouldGenerateCreateWebhook) {
        const webhookHostname = config.server.webhookHostname;
        const webhookURL = new URL(`/webhook/${webhookId}/github/`, webhookHostname).toString();
