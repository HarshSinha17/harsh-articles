...
export const mockAllProjectRoutes = async ({
    ...
    filteredRoutes.forEach((routerName) => {
        jest.mock(resolve(process.cwd(), `src/routers/routes/project/${routerName}`), () => {
            const original = jest.requireActual(resolve(process.cwd(), `src/routers/routes/project/${routerName}`));
            const mockedMethods = Object.keys(original).reduce((acc: any , methodName) => {
                acc[methodName] = jest.fn();
                return acc;
            }, {});
            return {
                __esModule: true,
                default: mockedMethods,
                ...mockedMethods
            };
        });
    });
